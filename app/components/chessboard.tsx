...
const Chessboard: React.FC = () => {
  ...
  const isBishopAttackSquare = (row: number, col: number, hoverRow: number, hoverCol: number): boolean => {
    return Math.abs(row - hoverRow) === Math.abs(col - hoverCol) && (row !== hoverRow || col !== hoverCol);
  };
  ...
  const renderSquare = (row: number, col: number) => {
    ...
    return (
      <div
        key={squareKey}
        className={`square ${isDarkSquare(row, col) ? 'dark' : 'light'} ${isHighlighted && 'highlight'} ${
          isHovered && 'hover'
        }`}
